...
Status: rev 5 · 2026-08-10 · authors: Claude (Opus 5, Fable 5), with Dave. Rev 5
adds the cost dimension the plan had ignored, and measures it rather than
assuming it: cost is turns times context depth, cold starts are negligible, and
§16.2's cold rounds turn out to be the cheap option (§16.3). Rev 4 settled the
two open naming questions and renamed throughout to plain terminology (§2
...
because it is read-only and derives entirely from the pm repo — one checkout
renders everything.
...
lives in a data file, so the churn of P0–P4 lands in TOML, not in recompiles.
...
| P4    | PM    | Ticket schema, `pm new/show/set/move/list/lint`, state machine. Tickets by hand only.                                                                                                                                                                   | A ticket cannot enter an invalid state                                    |
| P5    | PM    | **Dashboard.** `pm board` → HTML.                                                                                                                                                                                                                       | Dave says the board answers "where am I up to?"                           |
